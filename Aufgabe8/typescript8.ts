...
document.querySelector(".b4").addEventListener("click", function (): void {playSample(samp[3]); });
document.querySelector(".b5").addEventListener("click", function (): void {playSample(samp[4]); });
document.querySelector(".b6").addEventListener("click", function (): void {playSample(samp[5]); });
document.querySelector(".b7").addEventListener("click", function (): void {playSample(samp[6]); });
document.querySelector(".b8").addEventListener("click", function (): void {playSample(samp[7]); });
document.querySelector(".b9").addEventListener("click", function (): void {playSample(samp[8]); });
...
function playSample (samp: 0): void {
    while (i < beat.length) {
    samp[i].play();
    i++;
}}
...
// Play-Button (Beat)
document.querySelector(".fas fa-play-circle").addEventListener("click", function () { playBeat(beat: 0); // was soll denn dann hier in die Klanmnmer rein?
                                                                                      function playBeat (beat: 0): void { // ich verstehe die automatische Behebung nicht - ist doch genau das Gleiche?
    while (beat[i] == 0) {
        beat[i].play();
        i++;
    }}
